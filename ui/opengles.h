...
#define OPENGL_EMU_ERR(...)         {fprintf(stderr, __VA_ARGS__); fflush(stderr);}

/* #define ENABLE_DEBUG */
#ifdef ENABLE_DEBUG
    #define OPENGL_EMU_DBG(...)     {fprintf(stdout, __VA_ARGS__); fflush(stdout);}
    #define OPENGL_EMU_LOG(...)     {fprintf(stdout, __VA_ARGS__); fflush(stdout);}
#else
    #define OPENGL_EMU_DBG(...)     ((void)0)
    #define OPENGL_EMU_LOG(...)     ((void)0)
#endif
 ...
 * may be NULL.
 */
int android_startOpenglesRenderer(void);
...
int android_showOpenglesWindow(void);

int android_hideOpenglesWindow(void);

int android_redrawOpenglesWindow(int force);

int android_getOpenGLRendererStatus(void);
