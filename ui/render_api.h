...
/* Get the status of the OpenGL Renderer thread */
DECL(int, getOpenGLRendererStatus, (void));
 ...
 *     x,y,width,height are the dimensions of the rendering subwindow.
 *     zRot is the rotation to apply on the framebuffer display image.
 *     force will create the new window
 */
DECL(int, repaintOpenGLDisplay, (FBNativeWindowType window,
		int x, int y, int width, int height, float zRot, int force));
